defmodule Day3 do
  ...
  def calculate_distance(path1, path2) do
    ...
    {distance1, distance2} =
      Enum.map(shared_points, fn point ->
        {distance_to_point(path1_points_and_distance, point),
         distance_to_point(path2_points_and_distance, point)}
      end)
  ...
  def move(position_list, movement) do
    ...
    case direction do
      "R" ->
        Enum.map(oldX..(oldX + magnitude), fn x -> {x, oldY, current_distance + (x - oldX)} end)

      "L" ->
        Enum.map(oldX..(oldX - magnitude), fn x -> {x, oldY, current_distance + (oldX - x)} end)

      "U" ->
        Enum.map(oldY..(oldY + magnitude), fn y -> {oldX, y, current_distance + (y - oldY)} end)

      "D" ->
        Enum.map(oldY..(oldY - magnitude), fn y -> {oldX, y, current_distance + (oldY - y)} end)
    end
  end
